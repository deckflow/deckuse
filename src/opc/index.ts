...
const withUtf8EncodingDecl = (xml: string): string =>
  xml.replace(/^(\s*<\?xml\b[^?]*?)\bencoding\s*=\s*(['"])[^'"]*\2/i, `$1encoding=$2UTF-8$2`);
...
export class OpcArchive {
    ...
    const walk = async (dir: string, prefix = ''): Promise<void> => {
      for (const entry of await readdir(dir, { withFileTypes: true })) {
        ...
        if (total > limits.maxTotalSize)
          throw new Error('OPC total uncompressed size limit exceeded');
  ...
  writeXml(name: string, document: Node, mediaType?: string): void {
    ...
    const resolved =
      mediaType ?? this.contentTypes.overrides.get(normalized) ?? 'application/octet-stream';
