...
const resolveStyleChain = (archive: OpcArchive, slidePart: string): StyleChain => {
  ...
  if (!themePart) {
    const theme = [...archive.parts.keys()].find(
...
const txStyleLevel = (
  ...
  if (level === 0)
    return (
      directChild(block, 'defPPr') ??
      first(block, 'defPPr') ??
      directChild(block, 'lvl1pPr') ??
      first(block, 'lvl1pPr')
    );
...
const readFontFromRPr = (
  ...
  const color = solid ? (readSrgb(solid) ?? readSchemeColor(solid, themeColors)) : undefined;
  return {
    ...(family ? { family } : {}),
...
export function resolveProperties(
  ...
  all['visible'] = prop(!hidden, !hidden, {
    scope: 'local',
    target: resolved.target,
    path: 'visible',
  });
  ...
  if (ln && directChild(ln, 'noFill')) {
    ...
    all['line.color'] = prop(lineFill?.color ?? null, lineFill?.color ?? null, {
      scope: 'local',
      target: resolved.target,
      path: 'line.color',
    });
  ...
  if (!directFont.size || !directFont.family || !directFont.color) {
    ...
    if (ph && chain.layoutPart && archive.getPart(chain.layoutPart)) {
      ...
      if (layoutShape) {
        const layoutRPr =
          firstRunRPr(layoutShape) ??
          defRPrFromLevel(
            first(layoutShape, 'lstStyle')
              ? first(first(layoutShape, 'lstStyle')!, 'defPPr')
              : undefined,
          );
  ...
  all['hyperlink'] = prop(
    hlink,
    hlink,
    hlink ? { scope: 'local', target: resolved.target, path: 'hyperlink' } : { scope: 'default' },
  );
  ...
  for (const [key, value] of Object.entries(properties)) {
    switch (key) {
      ...
      case 'font.color':
        ...
        break;
      case 'fill.color':
        out['fill'] = typeof value === 'string' ? value.replace(/^#/, '') : value;
        break;
      case 'fill.kind':
