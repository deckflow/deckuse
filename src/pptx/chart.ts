...
import type { OpcArchive, OpcRelationship } from '../opc/index.js';
...
const plotXml = (
  ...
  if (chartType === 'combo') {
    ...
    const lineOnSecondary = useSecondary
      ? take(secondaryLine.length ? secondaryLine : lineSeries)
      : '';
  ...
  const ser = seriesXml(series, categories, formatCode !== undefined ? { formatCode } : {});
  if (chartType === 'pie') {
...
export function applyChartMajorGridlines(
  chartDoc: Document,
  show: boolean,
  color = 'FFFFFF',
): void {
...
export function applyChartProperties(
  ...
  if (typeof properties['gapWidth'] === 'number')
    applyChartGapWidth(chartDoc, properties['gapWidth']);
