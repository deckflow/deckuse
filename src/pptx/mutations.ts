...
const mapTextBlock = (block: Record<string, unknown>): TextBlockStyle => {
  ...
  if (typeof block['fontSize'] === 'number')
    (styled as { fontSize?: number }).fontSize = block['fontSize'];
  ...
  if (typeof block['italic'] === 'boolean')
    (styled as { italic?: boolean }).italic = block['italic'];
...
const copyXfrmInto = (doc: Document, seed: Element, ns: string, qname: string): Element => {
...
const ensureXfrm = (node: Element, archive?: OpcArchive, partUri?: string): Element => {
  const existing = (() => {
...
const transform = (
  ...
  if (needsExt && ext) {
    if (typeof t['width'] === 'number') ext.setAttribute('cx', String(Math.round(t['width'])));
    else if (!ext.hasAttribute('cx')) ext.setAttribute('cx', '0');
    if (typeof t['height'] === 'number') ext.setAttribute('cy', String(Math.round(t['height'])));
...
const materializeNotes = (archive: OpcArchive, item: IndexedElement): Result<IndexedElement> => {
...
export async function mutate(
  ...
  if (command.type === 'addSlide') {
    ...
    if (!layout) {
      const anchor =
        command.after !== undefined && command.after > 0 ? slides[command.after - 1] : slides[0];
      if (anchor) {
  ...
  if (command.type === 'setText') {
    if (command.blocks !== undefined) {
      if (chartPart) return err('INVALID_COMMAND', 'Chart titles do not support rich text blocks');
      setNodeTextBlocks(
        node,
        command.blocks.map((block) => mapTextBlock(block as Record<string, unknown>)),
      );
    ...
    transform(
      shapeNode,
      normalizeTransformFields(archive, command.transform as Record<string, unknown>),
      {
        archive,
    ...
    if (liveItem.kind === 'picture')
      detachPictureAndCleanup(archive, liveItem.partUri, doc, shapeNode);
