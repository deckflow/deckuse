...
): Promise<Result<{ mediaPart: string; mediaRid: string; fileRid: string; posterRid: string }>> {
  let data: Uint8Array;
  try {
...
export function mediaPicXml(
  ...
  const fileTag =
    kind === 'video' ? `<a:videoFile r:link="${fileRid}"/>` : `<a:audioFile r:link="${fileRid}"/>`;
...
export function detachMediaAndCleanup(
  ...
  const rids = [
    mediaFileRid(mediaNode),
    mediaEmbedRid(mediaNode),
    posterEmbedRid(mediaNode),
