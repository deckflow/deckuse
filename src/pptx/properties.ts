...
const setStroke = (spPr: Element, value: unknown): void => {
  ...
  const headEnd =
    typeof value === 'object' && value !== null
      ? (value as Record<string, unknown>)['headEnd']
      : undefined;
  const tailEnd =
    typeof value === 'object' && value !== null
      ? (value as Record<string, unknown>)['tailEnd']
      : undefined;
...
const runPropertyTargets = (node: Element): Element[] => {
  ...
  const runs = node.localName === 'r' ? [node] : descendants(node, 'r');
...
const setParagraphAlign = (node: Element, align: string): void => {
  ...
  const paragraphs = node.localName === 'p' ? [node] : paragraphNodes(node);
