...
export type NormalizePlaceholderRoleResult =
  { ok: true; type: PlaceholderType } | { ok: false; message: string };
...
export function normalizePlaceholderRole(role: string): NormalizePlaceholderRoleResult {
  ...
  if (!trimmed) return { ok: false, message: 'Placeholder role must be a non-empty string' };

  if (PLACEHOLDER_TYPE_SET.has(trimmed)) return { ok: true, type: trimmed as PlaceholderType };
