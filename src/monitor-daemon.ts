...
import { monitorDir } from './workspace/index.js';
...
export async function monitorStop(
  workspace: string,
): Promise<{ stopped: boolean; meta: DaemonMeta | null }> {
  const abs = resolve(workspace);
  const meta = await readDaemonMeta(abs);
  ...
  if (!target) {
    ...
    const disc = (await discoverMonitorsFromProcessTable()).find(
      (d) => resolve(d.workspace) === abs,
    );
    if (disc) {
...
export async function monitorStart(
  ...
  const child = spawn(
    process.execPath,
    [bin, 'monitor', '--daemon-worker', '--workspace', abs, '--host', host, '--port', String(port)],
    {
      detached: true,
      stdio: 'ignore',
      env: process.env,
