...
describe('pptx adapter', () => {
    ...
    const styled = await pptxAdapter.execute(
      {
        ...
        properties: {
          ...
          series: [{ name: '收入', values: [11, 22], color: '#4DA3FF' }],
        },
      },
      {},
    );
  ...
  it('addSlide --after inserts at the requested index and keeps content types', async () => {
    ...
    const presentation = await readFile(join(workspace, 'source/ppt/presentation.xml'), 'utf8');
    const ids = [...presentation.matchAll(/r:id="([^"]+)"/g)].map((m) => m[1]);
    expect(ids.length).toBe(3);

    const rels = await readFile(join(workspace, 'source/ppt/_rels/presentation.xml.rels'), 'utf8');
    const byId = new Map(
    ...
    const slideOverrides = [
      ...ct.matchAll(/PartName="(\/ppt\/slides\/slide\d+\.xml)" ContentType="([^"]+)"/g),
    ];
    expect(slideOverrides.length).toBeGreaterThanOrEqual(2);
    for (const [, , type] of slideOverrides) {
      expect(type).toBe('application/vnd.openxmlformats-officedocument.presentationml.slide+xml');
    }
  ...
  it('addSlide ignores p14:sectionLst sldId when syncing app.xml Slides count', async () => {
    ...
    const afterRemove = await readFile(join(workspace, 'source/ppt/presentation.xml'), 'utf8');
  ...
  it('init and setText heal stale app.xml Slides count (pre-existing mismatch)', async () => {
    ...
    const slideXml = `<p:sld xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"><p:cSld><p:spTree><p:nvGrpSpPr><p:cNvPr id="1" name=""/><p:cNvGrpSpPr/><p:nvPr/></p:nvGrpSpPr><p:grpSpPr/><p:sp><p:nvSpPr><p:cNvPr id="2" name="Title"/><p:cNvSpPr/><p:nvPr/></p:nvSpPr><p:spPr/><p:txBody><a:bodyPr/><a:lstStyle/><a:p><a:r><a:t>Hi</a:t></a:r></a:p></p:txBody></p:sp></p:spTree></p:cSld></p:sld>`;
    a.setPart(
      '/ppt/slides/slide1.xml',
      e.encode(slideXml),
      'application/vnd.openxmlformats-officedocument.presentationml.slide+xml',
  ...
  it('addSlide layout=blank binds to a Blank layout by cSld name, not the first layout', async () => {
    ...
    const rels = await readFile(join(workspace, 'source/ppt/slides/_rels/slide2.xml.rels'), 'utf8');
  ...
  it('placeholder hyperlink table paragraph notes phase1b writes', async () => {
    ...
    const rels = await readFile(join(workspace, 'source/ppt/slides/_rels/slide1.xml.rels'), 'utf8');
