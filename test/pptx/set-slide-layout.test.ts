...
import { naturalCompare, orderedLayouts } from '../../src/pptx/layout-ref.js';
import type { IndexFile } from '../../src/pptx/types.js';
...
describe('setSlideLayout', () => {
  it('rebinds by index, layout:N, slide:N, and preserves shape text', async () => {
    ...
    if (!listed.ok) return;
    const layouts = (
      listed.value as { items: Array<{ index: number; name: string; displayName: string }> }
    ).items;
    expect(layouts.map((l) => l.name)).toEqual(['slideLayout1', 'slideLayout2', 'slideLayout10']);
  ...
  it('addSlide accepts layout index and slide:N', async () => {
    ...
    const rels3 = await readFile(
      join(workspace, 'source/ppt/slides/_rels/slide3.xml.rels'),
      'utf8',
    );
    ...
    const rels4 = await readFile(
      join(workspace, 'source/ppt/slides/_rels/slide4.xml.rels'),
      'utf8',
    );
