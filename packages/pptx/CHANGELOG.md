# @deckflow/deckuse-pptx

## 1.2.0

### Minor Changes

- 6d11f42: Agent UX: length units, rich text blocks, align/distribute, table auto/theme, chart labels/combo, monitor daemon
- Same-batch forward refs; inline fill/stroke/blocks; multi-run text; wrap/anchor; cornerRadius; arrow/elbow/curved connectors; run-focused setProperties
- b085a32: Extract `@deckflow/deckuse-edition-config` so community and commercial builds share pptx/CLI source and only swap edition capability constants.

### Patch Changes

- Updated dependencies [6d11f42]
- Updated dependencies [b085a32]
  - @deckflow/deckuse-core@1.2.0
  - @deckflow/deckuse-edition-config@1.2.0
  - @deckflow/deckuse-workspace@1.1.1

## 1.1.0

### Minor Changes

- add monitor feature

### Patch Changes

- Updated dependencies
  - @deckflow/deckuse-core@1.1.0
  - @deckflow/deckuse-opc@1.1.0
  - @deckflow/deckuse-workspace@1.1.0

## 1.0.0

### Major Changes

- readme update, node.js version update

### Patch Changes

- Updated dependencies
  - @deckflow/deckuse-core@1.0.0
  - @deckflow/deckuse-opc@1.0.0

## 0.1.0

### Minor Changes

- publish

### Patch Changes

- Updated dependencies
  - @deckflow/deckuse-core@0.1.0
  - @deckflow/deckuse-opc@0.1.0
