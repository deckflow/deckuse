import { EMU_PER_PT, measureText } from '@deckflow/deckuse-core';
...
export const estimateTableHeightEmu = (
  rowCount: number,
  fontPt = TABLE_DEFAULT_FONT_PT,
): number => Math.round(Math.max(1, rowCount) * estimateTableRowHeightEmu(fontPt) * TABLE_HEIGHT_SAFETY);
...
export function measureTableLayout(input: TableMeasureInput): TableMeasureResult {
  ...
  const rowHeightsEmu = rows.map((row, rowIndex) => {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      const cellH =
        measured.heightEmu + TABLE_CELL_PAD_Y_EMU * 2 + TABLE_CELL_BORDER_EMU * 2;
