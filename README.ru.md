...
Сейчас реализованы форматы PPTX и DOCX. Адаптеры XLSX, Keynote и Numbers намеренно возвращают `FORMAT_NOT_IMPLEMENTED`; пока они не поддерживаются как цели редактирования.
...
Deckuse позволяет изменять существующую презентацию, не создавая её заново. Если исходного файла нет, `deckuse new` создаёт рабочее пространство из встроенного пустого шаблона. Рабочий процесс намеренно ориентирован на структуру, а не на визуальное представление:
...
По возможности сохраняются неизменённые XML и неизвестные части пакета. Это не полноценный движок рендеринга PowerPoint: используйте `monitor` для живого HTML-просмотра и `render` для PNG одного слайда. Семантические `diff` / `branch` пока отложены.
...
Это QA содержимого и структуры, а не визуальный QA. Используйте `render` / `monitor` только как помощь для проверки; Deckuse не обнаруживает перекрытия и не оценивает качество макета.
...
Инициализируйте новое рабочее пространство из одного исходника для каждого варианта. Для каждого варианта нужны собственные файл команд и путь вывода.
...
- Add, duplicate, remove, and rebind slides to another layout (`setSlideLayout` / `deckuse set slide-layout`); layout refs accept index, `layout:N`, `slide:N`, display name, or basename. Rebind updates the relationship only (does not edit layout parts). Duplicated slides clone mutable notes and chart parts while layouts and media can be shared safely.
